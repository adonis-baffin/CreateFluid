...
public class CopperSinkRenderer extends SmartBlockEntityRenderer<CopperSinkBlockEntity> {
    ...
    protected void renderSafe(CopperSinkBlockEntity be, float partialTicks, PoseStack ms,
        ...
        // Use interpolated fluid level instead of raw amount
        float level = be.getRenderedFluidLevel(partialTicks);
        if (level <= 0) return;

        FluidStack fluid = new FluidStack(Fluids.WATER, (int)(level * 2000));
        ...
        renderSide(vb, ms, x1, topY, z1, x2, topY, z1, x2, yBot, z1, x1, yBot, z1, sprite, color, light); // North
        renderSide(vb, ms, x2, topY, z2, x1, topY, z2, x1, yBot, z2, x2, yBot, z2, sprite, color, light); // South
        renderSide(vb, ms, x1, topY, z1, x1, topY, z2, x1, yBot, z2, x1, yBot, z1, sprite, color, light); // West
        renderSide(vb, ms, x2, topY, z2, x2, topY, z1, x2, yBot, z1, x2, yBot, z2, sprite, color, light); // East
    ...
    private void renderSide(VertexConsumer vb, PoseStack ms,
        ...
        var pose = ms.last();
        vb.addVertex(pose.pose(), x1, y1, z1).setColor(r, g, b, a).setUv(u0, v0).setLight(light).setNormal(0, 1, 0);
        vb.addVertex(pose.pose(), x2, y2, z2).setColor(r, g, b, a).setUv(u1, v0).setLight(light).setNormal(0, 1, 0);
        vb.addVertex(pose.pose(), x3, y3, z3).setColor(r, g, b, a).setUv(u1, v1).setLight(light).setNormal(0, 1, 0);
        vb.addVertex(pose.pose(), x4, y4, z4).setColor(r, g, b, a).setUv(u0, v1).setLight(light).setNormal(0, 1, 0);
    }
}
