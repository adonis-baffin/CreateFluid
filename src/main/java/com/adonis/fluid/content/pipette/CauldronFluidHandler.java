...
public class CauldronFluidHandler implements IFluidHandler {
    ...
    public FluidStack getFluidInTank(int tank) {
        ...
        if (state.is(Blocks.POWDER_SNOW_CAULDRON)) {
            int level = state.getValue(LayeredCauldronBlock.LEVEL);
            if (level == 3) { // 只有满的细雪炼药锅才返回流体
                return CFFluid.getPowderSnowFluidStack(FULL_AMOUNT);
            }
            return FluidStack.EMPTY;
        }
    ...
    public int fill(FluidStack resource, FluidAction action) {
        ...
        if (state.is(Blocks.CAULDRON)) {
            ...
            } else if (CFFluid.isPowderSnowFluid(resource.getFluid())) {
                // 细雪：必须一次性填满1000mB
                if (resource.getAmount() >= FULL_AMOUNT) {
    ...
    public FluidStack drain(FluidStack resource, FluidAction action) {
        ...
        if (!contained.isFluidEqual(resource)) return FluidStack.EMPTY;
    ...
    public FluidStack drain(int maxDrain, FluidAction action) {
        ...
        // 细雪炼药锅 - 只能抽取满的，一次性抽取1000mB
        else if (state.is(Blocks.POWDER_SNOW_CAULDRON)) {
            int currentLevel = state.getValue(LayeredCauldronBlock.LEVEL);
            if (currentLevel == 3 && maxDrain >= FULL_AMOUNT) {
                result = CFFluid.getPowderSnowFluidStack(FULL_AMOUNT);
