...
public class CopperTapScenes {
    ...
    public static void tap(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.world().modifyBlockEntity(tank2BottomPos, FluidTankBlockEntity.class, be -> {
            be.getTankInventory().fill(CFFluid.getPowderSnowFluidStack(4000), IFluidHandler.FluidAction.EXECUTE);
        });
        ...
        for (int i = 0; i < 30; i++) {
            ...
            createFluidStream(scene, tapBottom, depotTop, CFFluid.getPowderSnowFluidStack(100));
            scene.idle(1);
        }
    ...
    private static void simulateItemFilling(CreateSceneBuilder scene, BlockPos tapPos, BlockPos depotPos,
        ...
        for (int i = 0; i < 20; i++) {
            createFluidStream(scene, tapBottom, depotTop, powderSnow);
            scene.idle(1);
        }
    ...
    private static void createFluidStream(CreateSceneBuilder scene, Vec3 start, Vec3 end, FluidStack fluid) {
        ...
        for (float t = 0; t <= 1; t += 0.1f) {
            ...
            Vec3 motion = flow.normalize().scale(0.01); // 更小的速度，让粒子更密集

            // 细流效果，减少随机偏移
            RandomSource random = RandomSource.create();
            pos = pos.add(
                    VecHelper.offsetRandomly(Vec3.ZERO, random, 0.005f) // 很小的偏移，保持细流形状
            );

            scene.effects().emitParticles(
                    pos,
                    scene.effects().simpleParticleEmitter(particle, motion),
                    0.5f, // 减少粒子数量，但由于更密集的生成点，效果更像细流
                    1
            );
        }
